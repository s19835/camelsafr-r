...
test_that("download_parquet errors on non-zero exit status", {
  dest <- file.path(tempfile(), "test.parquet")
  mockery::stub(download_parquet, "utils::download.file",
                function(...) invisible(1L))
  expect_error(
    download_parquet("https://example.com/test.parquet", dest),
    "Failed to download.*exit status 1"
  )
})

test_that("read_parquet_url errors when cache=TRUE and download fails", {
  tmp <- tempfile(fileext = ".parquet")  # does not exist
  mockery::stub(read_parquet_url, "cache_path", function(...) tmp)
  mockery::stub(read_parquet_url, "download_parquet", function(url, dest) {
    stop("Download failed")
  })
  expect_error(
    read_parquet_url("https://example.com/fake.parquet", cache = TRUE),
    "Download failed"
  )
})
